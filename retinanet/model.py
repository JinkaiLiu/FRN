...
import random
from retinanet.fusion import REFusion
...
class ResNet_fpn_fusion(nn.Module):
    ...
    def __init__(self, num_classes, event_block, rgb_block, event_layers, rgb_layers):
        #rgb branch
        self.inplanes = 64
        ...
        self.fpn = PyramidFeatures(rgb_fpn_sizes[0]+event_fpn_sizes[0], rgb_fpn_sizes[1]+event_fpn_sizes[1], rgb_fpn_sizes[2]+event_fpn_sizes[2], rgb_fpn_sizes[3]+event_fpn_sizes[3])
    ...
    def forward(self, inputs):
        ...
        x1 = self.fus[0](x1_event, x1_rgb)
        x2 = self.fus[1](x2_event, x2_rgb)
        x3 = self.fus[2](x3_event, x3_rgb)
        x4 = self.fus[3](x4_event, x4_rgb)
        ...
        if self.training:
            ...
            for i in range(classification.shape[2]):
                ...
                finalAnchorBoxesIndexes = torch.cat((finalAnchorBoxesIndexes, finalAnchorBoxesIndexesValue)) #torch.cat是将两个张量（tensor）拼接在一起
...
class ResNet_rgb(nn.Module):
    ...
    def __init__(self, num_classes, block, layers):
        ...
        self.fpn = PyramidFeatures(fpn_sizes[0], fpn_sizes[1], fpn_sizes[2], fpn_sizes[3]) #C3_size, C4_size, C5_size
...
def resnet50(num_classes, fusion_model, pretrained=False, **kwargs):
    ...
    if fusion_model == 'fpn_fusion':
        model = ResNet_fpn_fusion(num_classes, event_block=Bottleneck, rgb_block=Bottleneck, event_layers=[3, 4, 6, 3],rgb_layers=[3, 4, 6, 3], **kwargs)
