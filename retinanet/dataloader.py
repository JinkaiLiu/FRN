...
import torch
import skimage.transform #进行图像处理：图像的形变与缩放
...
class CSVDataset_event(Dataset): #只用了left的图
    """CSV dataset."""
    ...
    def __init__(self, train_file, class_list, root_event_dir, root_img_dir, transform=None):
        ...
        try:
            with self._open_for_csv(self.class_list) as file:
                self.classes = self.load_classes(csv.reader(file, delimiter=',')) #分隔符是“，”
        ...
        try:
            with self._open_for_csv(self.train_file) as file:
                self.image_data = self._read_annotations(csv.reader(file, delimiter=','), self.classes) ##read_annotations返回一个字典，key是文件名，bbox和class是value。
        except ValueError as e:
            raise (ValueError('invalid CSV annotations file: {}: {}'.format(self.train_file, e)))
        self.image_names = list(self.image_data.keys()) #一个列表
    ...
    def load_image(self, image_index):
        ...
        img = torch.from_numpy(np.load(event_file)['arr_0']) #npz包含多个数组，默认用arr_0，arr_1的方式来访问对应的数组
        img = img.permute(1,2,0) # 将(5,480,640)转化为(480,640,5)
        ...
        img_rgb = img_rgb.astype(np.float32)/255.0 #处理成float形式并缩放到0-1,原始图像是0-256之间的
    ...
    def _read_annotations(self, csv_reader, classes):
        #返回一个字典，key是文件名，bbox和class是value。
        result = {}
        for line, row in enumerate(csv_reader):
            line += 1
...
def collater(data):
    #通过batch_sampler采样得到的样本数据，其图像尺寸可能不完全一致，这时就需要用到collate_fn参数指定被采样样本图片尺寸的调整方式。
    #通常的做法是，获得这组样本中图片尺寸的最大值 Widthmax和Heightmax，然后将改组样本中所有图像的尺寸调整 Heightmax×Widthmax 最终返回图像数据为： BatchSize×Heightmax×Widthmax×3
    #此外，每个样本中的BoundingBoxs的数量也可能不同，设BBs数量最大值为 Annmax ，也要将标签和类型尺寸调整相同，对于BBs小于 Annmax 的样本，补充-1。最终返回标签数据为：BatchSize×Annmax×5
...
#该函数的目的是为给DataLoader()函数传递batch_sample参数
