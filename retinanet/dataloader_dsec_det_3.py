...
class DSECDetDataset(Dataset):
    ...
    def _load_events(self, events_path, timestamp):
        try:
            npz_path = Path(events_path).with_suffix(".npz")
            if npz_path.exists():
            ...
            print(f"Events: {len(events_t)}, time range: [{events_t.min():.0f} - {events_t.max():.0f}]")

            # 使用事件序列的中间50%作为有效事件
            event_duration = events_t.max() - events_t.min()
            center_time = (events_t.min() + events_t.max()) / 2
            window = event_duration * 0.5  # 取中间50%
            mask = (events_t >= center_time - window/2) & (events_t <= center_time + window/2)
            print(f"Using middle 50%: {mask.sum()} events")
