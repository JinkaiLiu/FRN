...
class EventsCache:
    ...
    def get(self, events_path):
        if events_path in self.cache:
            self.cache_order.remove(events_path)
            self.cache_order.append(events_path)
            return self.cache[events_path]
        return None
    
    def put(self, events_path, events_data):
        if events_path in self.cache:
            self.cache_order.remove(events_path)
        ...
        self.cache[events_path] = events_data
        self.cache_order.append(events_path)
...
class DSECDetDataset(Dataset):
    def __init__(self, root_dir, split='train', transform=None, event_representation='time_surface', 
                 ...
                 use_downsampled_events=True, cache_events=True):
        ...
        if cache_events:
            self.events_cache = EventsCache(cache_size=20)
    ...
    def _load_samples(self):
        ...
        for seq in self.sequences:
            ...
            if self.use_downsampled_events:
                events_file = seq_path / 'events' / 'left' / 'events_2x.npz'
                if not events_file.exists():
                    events_file = seq_path / 'events' / 'left' / 'events_2x.h5'
            else:
                events_file = seq_path / 'events' / 'left' / 'events.npz'
                if not events_file.exists():
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            img_event = self._load_events_npz(sample['events_path'], sample['timestamp'])
    ...
    def _load_events_npz(self, events_path, timestamp):
        try:
            events_path_obj = Path(events_path)
            
            if self.cache_events:
                cached_events = self.events_cache.get(events_path)
                if cached_events is not None:
                    events_x, events_y, events_t, events_p = cached_events
                else:
                    events_x, events_y, events_t, events_p = self._load_events_from_file(events_path_obj)
                    if self.cache_events:
                        self.events_cache.put(events_path, (events_x, events_y, events_t, events_p))
            else:
                events_x, events_y, events_t, events_p = self._load_events_from_file(events_path_obj)
            
            if self.debug:
                print(f"总事件数量: {len(events_x)}")
                print(f"事件时间范围: [{events_t.min()}, {events_t.max()}]")
            ...
            if self.debug:
                print(f"时间窗口: [{start_time}, {end_time}] (窗口大小: {dt_us} us)")
            
            mask = (events_t >= start_time) & (events_t < end_time)
            num_events_in_window = mask.sum()
            
            if self.debug:
                print(f"窗口内事件数量: {num_events_in_window}")
            
            if num_events_in_window == 0:
                print(f"⚠️ 窗口内没有事件，尝试智能扩大时间窗口...")
    
                # 渐进式扩大窗口，保持数据质量
                expansion_steps = [
                    (100, "±100ms"),   # 首先扩大到±100ms
                    (200, "±200ms"),   # 然后±200ms  
                    (500, "±500ms"),   # 最后±500ms
                ]
                ...
                for step_dt_ms, step_name in expansion_steps:
                    step_dt_us = step_dt_ms * 1000
                    step_start = timestamp - step_dt_us
                    step_end = timestamp + step_dt_us
                    step_mask = (events_t >= step_start) & (events_t < step_end)
                    step_count = step_mask.sum()
        
                    print(f"  尝试{step_name}窗口: {step_count} 个事件")
        
                    if step_count > 100:  # 至少要有100个事件才有意义
                        # 进一步筛选：只选择最近的事件，保持时间局部性
                        time_diffs = np.abs(events_t[step_mask] - timestamp)
                        sorted_indices = np.argsort(time_diffs)
            
                        # 选择时间上最近的事件，数量根据窗口大小调节
                        max_events = min(2000, step_count, int(step_count * 0.5))
                        selected_indices = sorted_indices[:max_events]
            
                        # 创建最终的mask
                        step_indices = np.where(step_mask)[0]
                        final_indices = step_indices[selected_indices]
            
                        mask = np.zeros_like(events_t, dtype=bool)
                        mask[final_indices] = True
            
                        closest_time_diff = time_diffs.min()
                        print(f"  ✅ 使用{step_name}窗口: {mask.sum()} 个最近事件 (最近时间差: {closest_time_diff/1000:.1f}ms)")
                        found_events = True
                        break
    
                if not found_events:
                    print(f"❌ 所有扩展窗口都没有足够事件，返回空事件数据")
            ...
            if self.debug and len(x) > 0:
                print(f"最终事件统计: {len(x)} 个事件")
                print(f"空间范围: X[{x.min()}, {x.max()}], Y[{y.min()}, {y.max()}]")
                print(f"时间范围: [{t.min()}, {t.max()}]")
                print(f"极性分布: +{(p > 0).sum()}, -{(p <= 0).sum()}")
                    
        except Exception as e:
            print(f"❌ 事件加载错误 {events_path}: {e}")
            import traceback
            traceback.print_exc()
            return self._get_empty_events()
        
        event_img = create_robust_event_representation(
            x, y, t, p, self.image_height, self.image_width, self.normalize_events
        )
        
        if self.debug:
            print(f"事件图像统计: 形状={event_img.shape}, 范围=[{event_img.min():.3f}, {event_img.max():.3f}]")
            print(f"非零像素数量: {(event_img != 0).sum()}")
        
        return event_img
    
    def _load_events_from_file(self, events_path):
        if events_path.suffix == '.npz':
            return self._load_from_npz(events_path)
        elif events_path.suffix == '.h5':
            return self._load_from_h5(events_path)
        else:
            raise ValueError(f"Unsupported file format: {events_path.suffix}")
    
    def _load_from_npz(self, events_path):
        try:
            if self.debug:
                print(f"📂 加载NPZ文件: {events_path}")
                
            data = np.load(str(events_path))
            
            required_keys = ['x', 'y', 't', 'p']
            missing_keys = [k for k in required_keys if k not in data]
            if missing_keys:
                raise ValueError(f"NPZ文件缺少必要的键: {missing_keys}")
            ...
            if len(events_x) == 0:
                print(f"⚠️ NPZ文件中没有事件数据")
                return np.array([]), np.array([]), np.array([]), np.array([])
            
            if self.debug:
                print(f"✅ NPZ加载成功: {len(events_x)} 个事件")
                print(f"时间范围: [{events_t.min()}, {events_t.max()}]")
                print(f"空间范围: X[{events_x.min()}, {events_x.max()}], Y[{events_y.min()}, {events_y.max()}]")
                print(f"极性范围: [{events_p.min()}, {events_p.max()}]")
            
            return events_x, events_y, events_t, events_p
            
        except Exception as e:
            print(f"❌ NPZ文件加载失败 {events_path}: {e}")
            raise e
    
    def _load_from_h5(self, events_path):
        try:
            import h5py
            
            if self.debug:
                print(f"📂 加载H5文件: {events_path}")
            ...
            with h5py.File(events_path, 'r') as f:
                ...
                if 't_offset' in f:
                    t_offset = f['t_offset'][()]
                    if self.debug:
                        print(f"✅ 应用t_offset: {t_offset}")
                    events_t = events_t + t_offset
                
                if len(events_x) == 0:
                    print(f"⚠️ H5文件中没有事件数据")
                    return np.array([]), np.array([]), np.array([]), np.array([])
                
                if self.debug:
                    print(f"✅ H5加载成功: {len(events_x)} 个事件")
                    print(f"时间范围: [{events_t.min()}, {events_t.max()}]")
                    print(f"空间范围: X[{events_x.min()}, {events_x.max()}], Y[{events_y.min()}, {events_y.max()}]")
            ...
            print(f"❌ H5文件加载失败 {events_path}: {e}")
            raise e
